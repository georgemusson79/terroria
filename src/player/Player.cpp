...
Player::Player(Vector2 pos) {
  ...
  //	this->armPos = { center.X - (armDims.X / 2),position.Y + (this->height *
  // 0.4) };
  this->arm = new Arm({0, -0.4}, {0, armDims.Y}, armDims.X, armDims.Y,
                      pathToDefaultArmTexture, false, this, this->position);
  this->arm2 = new Arm({0, -0.4}, {0, armDims.Y}, armDims.X, armDims.Y,
                       pathToDefaultArmTexture, false, this, this->position);

  // this->armTexture =
  // IMG_LoadTexture(Main::renderer,"assets\\player\\arm.png");
  this->hitboxes.push_back(new SquareHitbox(
...
void Player::update() {
  ...
  if (this->openedChest != nullptr &&
      this->position.distance({this->openedChest->X, this->openedChest->Y}) > 9)
  ...
  for (int x = 0; x < inventoryRows; x++) {
    for (int y = 0; y < inventoryColumns; y++) {
      if (inventory[x][y] == nullptr) {
        inventory[x][y] = item;
        return true;
      } else if (inventory[x][y]->id == item->id &&
                 (inventory[x][y]->count + item->count) <= item->maxStack) {
...
//	Vector2 dims = { Main::convertTileSizeToPixelSize(this->width *
// this->scale), Main::convertTileSizeToPixelSize(this->height * this->scale) };
//	SDL_Rect r = { renderPos.X,renderPos.Y,dims.X,dims.Y };
//	SDL_Rect partToRender = Main::getTextureFrame(this->animationFrame,
// this->frameCount, this->texture, this->textureWidth, this->textureHeight, 0,
// 0); 	SDL_RenderCopyEx(Main::renderer, this->texture, &partToRender, &r,
// this->rotation, &this->rotationPoint, this->getSpriteDirection()); 	if
//(Debug::renderHitboxes) this->renderHitboxes();
//
//	this->armDims= { this->width * 0.2,this->height * 0.4};
//	this->armPos = { center.X - (armDims.X / 2),position.Y + (this->height *
// 0.4) }; 	SDL_Point armdims = {
// Main::convertTileSizeToPixelSize(armDims.X),
// Main::convertTileSizeToPixelSize(armDims.Y) }; 	iVector2 armpos =
// Main::convertWorldPosToCameraPos({
// center.X-(armDims.X/2),position.Y+(this->height*0.4)}); 	SDL_Rect armRect
// = { armpos.X,armpos.Y,armdims.x,armdims.y }; 	SDL_Point rotationPt = {
// armRect.w/2, 0}; 	double rotation = armRotation * this->hDirection;
//	SDL_RenderCopyEx(Main::renderer, this->armTexture, NULL,
//&armRect,rotation,&rotationPt,this->getSpriteDirection()); 	Vector2 handPos
//= this->getHandPos(); 	Main::drawSquare(handPos, { 255,0,0 },
// armRect.w);
//
//
//	return true;
...
void Player::rightClick() {
  ...
  if (Main::world->tileAt(pos.X, pos.Y) != nullptr) {
    Main::world->tileAt(pos.X, pos.Y)->onRightClick(this);
    // Main::world->tileAt(pos.X, pos.Y)->destroy();
  }
}
...
void Player::leftClick() {
  Vector2 pos = Cursor::cursorWorldPos;
  if (Main::world->tileAt(pos.X, pos.Y) != nullptr)
    Main::world->tileAt(pos.X, pos.Y)->onLeftClick(this);
...
void Player::handleMouseInput() {
  ...
  if (mousePressed & SDL_BUTTON(SDL_BUTTON_LEFT)) {
    ...
    // if (mousePressed!=Main::heldMouseKeys &&
    // !Main::player->isCollidingWithGUI) Chest* h = new Chest(pos.X, pos.Y,
    // true);
  }
...
void Player::handleKeyInput() {
  ...
  if (keys[SDL_SCANCODE_A]) {
    this->velocity.X -=
        (this->velocity.X > -this->maxXVelocity) ? this->hAcceleration : 0;
    this->isWalking = true;
  }

  if (keys[SDL_SCANCODE_D]) {
    this->velocity.X +=
        (this->velocity.X < maxXVelocity) ? this->hAcceleration : 0;
    this->isWalking = true;
  }
...
bool Player::handleWalking() {
  ...
  if (this->animationTimePassed < 15)
    return false;
  PlayerSprite walkAnims[] = {PlayerSprite::WALK1, PlayerSprite::IDLE,
                              PlayerSprite::WALK2, PlayerSprite::IDLE};
  this->walkFrame++;
  ...
  if (!this->arm->usingItem) {
    ...
    this->arm2->rotation =
        (this->arm->rotation != 0) ? -this->arm->rotation : 0;
  }
  return true;
}
...
//
//
//	if (this->usingItem and this->swingItem!=nullptr and
// this->this->arm->rotation<300 and this->heldItem.useTime!=0) {
//	//bruh this code sucks
//		this->this->arm->rotation += double(270 - 90) /
// this->heldItem.useTime; 		double itemRotation =
// (this->arm->rotation + 180) * hDirection; 		if (this->swingItem !=
// nullptr) { 			this->swingItem->setRotation(0);
// Vector2 handPos
//= this->getHandPos(this->swingItem->handOffset); 			Vector2
// itemCenter = { handPos.X + (swingItem->width/2),handPos.Y +
// (swingItem->height/2) };
//			//move item center to account for rotation offset so
// that it remains in the players hand 			Vector2
// newPos=Main::rotatePt(itemCenter, handPos, this->arm->rotation*hDirection);
...
//}; 	Vector2 end = { base.X,base.Y + this->armDims.Y }; 	end =
// Main::rotatePt(end-offset, base, this->arm->rotation*this->hDirection);
// return end;
// }
...
bool Player::removeFromInventory(int row, int col, int amount) {
  if (this->inventory[row][col] == nullptr)
    ...
    this->inventory[row][col]->setCount(this->inventory[row][col]->getCount() -
                                        amount);
...
bool Player::has(int itemID, Vector2 *itemPos) {
  for (int x = 0; x < this->inventoryRows; x++) {
    for (int y = 0; y < this->inventoryColumns; y++) {
      if (this->inventory[x][y] != nullptr &&
          this->inventory[x][y]->id == itemID) {
        *itemPos = {x, y};
        return true;
      }
    }
...
bool Player::has(AmmoType ammoID, Vector2 *pos) {
  for (int x = 0; x < this->inventoryRows; x++) {
    for (int y = 0; y < this->inventoryColumns; y++) {
      if (this->inventory[x][y] != nullptr &&
          this->inventory[x][y]->ammoID == ammoID) {
        *pos = {x, y};
        return true;
      }
    }
