...
MultiTileObject::MultiTileObject(uint16_t id, uint16_t X, uint16_t Y,
                                 uint16_t W, uint16_t H, int health,
                                 bool isParent, MultiTileObject *parent)
    : Tile(id, X, Y, health) {
  this->width = W;
...
bool MultiTileObject::draw(SDL_Renderer *renderer, Camera &camera) {
  if (this->isParent) {
    if (tileID != AIR) {
      ...
      newX = std::round(newX);
      newY = std::round(newY);
      SDL_Rect dstrect = {newX, newY, camera.zoom() * this->width,
                          camera.zoom() * this->height};
      if ((dstrect.x < Main::WINDOW_WIDTH && dstrect.x + dstrect.w > 0) &&
          (dstrect.y < Main::WINDOW_HEIGHT && dstrect.y + dstrect.h > 0)) {
        double brightness =
            ((double)Main::lightMap[this->X][this->Y] / 15) * 255;
        if (brightness > 255)
          brightness = 255;
    ...
    return this->parent->draw(renderer, camera);
  return false;
}
  ...
  if (this->isParent) {
    for (Tile *tile : this->tileList) {
      if (Main::inWorldBounds({tile->X, tile->Y})) {
        ...
        Main::world->setTileAt(tile->X, tile->Y, nullptr);
...
void MultiTileObject::generate() {
  if (placeIntoWorld) {
    bool inRange = (this->X >= 0 && this->Y >= 0 &&
                    this->Y + this->height < Main::world->WORLD_HEIGHT &&
                    this->X + this->width < Main::world->WORLD_WIDTH);
    assert(inRange);
    ...
    for (int x = 0; x < width; x++) {
      for (int y = 0; y < height; y++) {
        if (x != 0 || y != 0) {
          ...
          cpy->Y += y;
          cpy->generate();
          this->tileList.push_back(Main::world->tileAt(cpy->X, cpy->Y));
        }
      }
    }
  }
...
bool MultiTileObject::mine(int pickaxePower, int axePower, int hammerPower,
                           bool dropItem) {
  if (this->parent != nullptr) {

    return this->parent->mine(pickaxePower, axePower, hammerPower, dropItem);
  } else
    return Tile::mine(pickaxePower, axePower, hammerPower, dropItem);
}
