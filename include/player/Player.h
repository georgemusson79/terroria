...
enum class PlayerSprite { WALK1, IDLE, WALK2 };
class Chest;
class Arm;
...
public:
  ...
  std::vector<std::shared_ptr<Item>> accessories =
      std::vector<std::shared_ptr<Item>>(accessoryCount, nullptr);
  ...
  bool pickup(ItemType *item, int row, int column, bool replace = false) {
    ...
    if (row < this->inventoryRows && column < this->inventoryColumns) {
      if (replace || inventory[row][column] == nullptr) {
        ...
        if (item->id == inventory[row][column]->id && item->isStackable &&
            (inventory[row][column]->count + item->count) <= item->maxStack) {
          inventory[row][column]->setCount(inventory[row][column]->count +
                                           item->count);
          return true;
        }
      }
  ...
  template <typename ItemType> bool pickup(ItemType *item) {
    static_assert(std::is_base_of_v<Item, ItemType>,
                  "item is not of type Item");
    ItemType cpy = *item;
