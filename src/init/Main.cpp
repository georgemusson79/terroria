// main.cpp
#include "Main.h"
#include "Bossbar.h"
#include "Colliders.h"
#include "Cursor.h"
#include "Entities.h"
#include "Item.h"
#include "Items.h"
#include "Player.h"
#include "Projectiles.h"
#include "Tile.h"
#include "Tiles.h"
#include "World.h"
#include "engineDebug.h"
#include "gui.h"
#include "tileWall.h"
#include "tileWalls.h"
#include "world.h"
#include <Windows.h>
#include <fstream>

#define SDL_MAIN_HANDLED
CircleHitbox *Main::testcbox = new CircleHitbox;
int Main::frames = 0;
int Main::WINDOW_WIDTH = 1920 / 2;
double Main::gravity = 9;
int Main::WINDOW_HEIGHT = 1080 / 2;
bool Main::scaleToWindowSize = false;
bool Main::doesBossBarExist = false;
std::vector<Entity *> Main::entities = {};
std::vector<Entity *> Main::entitiesToSpawn = {};
std::set<Tile *> Main::tilesToDelete = {};
std::map<int, SDL_Texture *> Main::tileTexture;
std::map<int, SDL_Texture *> Main::tileWallTexture;
std::map<int, SDL_Texture *> Main::backgrounds;
World *Main::world = new World();

uint16_t Main::defaultLightValue = 0;
std::vector<std::vector<uint16_t>>
    Main::lightMap(Main::world->WORLD_WIDTH + 1,
                   std::vector<uint16_t>(Main::world->WORLD_HEIGHT + 1,
                                         Main::defaultLightValue));
std::vector<iVector2> Main::changedLights;

SDL_Window *Main::window;
SDL_Renderer *Main::renderer;
SDL_Renderer *r2;
SDL_Cursor *Main::cursor;
bool Main::isSmartCursorEnabled = false;
Camera *Main::camera = nullptr;
Player *Main::player = nullptr;
Player *Main::p2 = nullptr;

int main(int argc, char **argv) {
  // this must happen first
  Main::init();
  // Main::world->valueNoiseGen(500, 30, 20);

  // Main::world->temp_populateHalfTiles(Dirt(2, 2, false), WoodWall(2, 2,
  // false));
  Main::world->temp_populateByValueGen(Dirt(2, 2, false),
                                       WoodWall(2, 2, false));
  Main::camera = new Camera(1000, 1000, Main::renderer);
  Main::player = new Player({500, Main::world->WORLD_HEIGHT / 2});
  Main::player->setY(Main::player->position.Y - 10);
  // Main::p2 = new Player({ 500,Main::world->WORLD_HEIGHT/2 });
  SDL_Event e;

  Main::testcbox->position = Main::player->position;
  Main::testcbox->r = 10;
  Main::testcbox->active = false;
  int starttime = SDL_GetTicks();
  int frmstart = SDL_GetTicks();
  bool increasing = true;
  // Entity1* en=new Entity1({ Main::player->position.X + 10,
  // Main::player->position.Y }); Entity1* en2=new Entity1(en->position);

  // std::ofstream f("C:\\Users\\george\\Pictures\\Screenshots\\tiles.bin",
  // std::ios::binary); Tile* t = new Dirt(1, 1,true); t->save(f);
  ////Main::world->tileAt(900, 900)->save(f);
  // f.close();

  // std::ofstream f("tiles.bin", std::ios::binary);
  // for (Tile* t : Main::tiles[0]) if (t!=nullptr && t->getID()!=AIR)
  // t->save(f); f.close();

  // std::ifstream f2("C:\\Users\\george\\Pictures\\Screenshots\\tiles.bin",
  // std::ios::binary); std::cout << f2.is_open() << "\n";
  ////for (int i = 0; i < 1; i++) {
  //    Tile* r = new Tile();
  //    std::cout << f2.read((char*)r, 40).gcount() << "\n";
  //    r->getInfo();

  //}

  while (Debug::running) {
    Main::removeDeletedTiles();
    Main::removeDeletedEntities();
    Main::spawnEntities();
    Main::resetLightMap();

    Cursor::update();
    Main::handleEvents(&e, nullptr);
    if (Main::player != nullptr) {

      Main::player->update();
      // Main::player->selectedHBItem++;
      // if (Main::player->selectedHBItem == 10) Main::player->selectedHBItem =
      // 0;
    }
    Main::testcbox->r += 0.01;
    for (int e = 0; e < Main::entities.size(); e++) {
      if (Main::entities[e] != nullptr && !Main::entities[e]->toBeDeleted() &&
          Main::entities[e]->active) {
        Entity *entity = Main::entities[e];
        float maxUpdateDist =
            Main::camera->diagonalLength +
            std::sqrt(std::pow(entity->width, 2) + std::pow(entity->height, 2));
        if (Main::entities[e]->alwaysUpdate ||
            (Main::player == nullptr ||
             Main::entities[e]->center.distance(Main::player->center)) <
                maxUpdateDist)
          Main::entities[e]->update();
      }
    }
    for (Tile *t : Main::testcbox->collidesWithTiles())
      t->destroy();

    Main::camera->renderScreen();
    // handle fps: should be put in function and needs work

    if (SDL_GetTicks() - frmstart < 1000 / 60) {

      // while (SDL_GetTicks() - frmstart < 1000 / 60 && Main::vsync) {}
      frmstart = SDL_GetTicks();
    } else {
      frmstart = SDL_GetTicks();
    }
    Main::frames++;
    if (SDL_GetTicks() - starttime > 10000) {
      std::cout << Main::frames / 10 << "\n";
      Main::frames = 0;
      starttime = SDL_GetTicks();
    }
    Main::doesBossBarExist = false;
  }

  return 0;
}
