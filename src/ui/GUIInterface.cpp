...
void GUIinterface::InventoryTile::renderTile()
{
	...
	if (this->item != nullptr)
	{
		...
		itemRect.x = outputRect.x + margin;
		itemRect.y = outputRect.y += margin;
		itemRect.w = size - (2 * margin);
		itemRect.h = size - (2 * margin);
...
void GUIinterface::renderPlayerInventory(Player *player)
{
	...
	for (int x = 0; x < player->inventoryRows; x++)
	{
		for (int y = 0; y < player->inventoryColumns; y++)
		{
			...
			if (x == 0 && y == player->selectedHBItem)
			{
				...
				tile = new GUIinterface::InventoryTile(InventoryFrames::PLAYER_INVENTORY, pos, scale, Main::player->inventory[x][y]);
			pos.x += scale + gap;
			tile->renderTile();
			delete tile;
		}
	...
	if (player->openedChest != nullptr)
		player->openedChest->renderInventory(pos);
}
...
void GUIinterface::renderPlayerHotbar(Player *player)
{
	...
	for (int i = 0; i < player->inventoryColumns; i++)
	{
		...
		if (i == player->selectedHBItem)
		{
			...
			tile = new GUIinterface::InventoryTile(InventoryFrames::PLAYER_INVENTORY, pos, scale, Main::player->inventory[0][i], SpecialisedType::GENERIC, i, true);
		pos.x += scale + gap;
		tile->renderTile();
		delete tile;
	}
...
void GUIinterface::InventoryTile::update()
{
	if (this->hitbox->collidesWith(Cursor::cursorWorldPos))
	{
		...
		if (Main::player->inventoryOpen)
			this->type = InventoryFrames::PLAYER_SELECTED_INVENTORY;

		if (Cursor::isLeftPressed())
		{
			if (SDL_GetTicks() - Cursor::swapItemCooldown > 500 && !this->isHotbar)
				Cursor::swapItem(this);
