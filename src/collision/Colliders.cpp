...
void Hitbox::setPosition(Vector2 position) { this->position = position; }

CircleHitbox::CircleHitbox(Vector2 position, float radius)
    : Hitbox::Hitbox(hitboxType::CIRCLE) {
...
bool CircleHitbox::collidesWith(Vector2 pos) {
  if (pos.distance(this->position) < this->r && this->active)
    return true;
  return false;
}

Vector2 CircleHitbox::getCenter() { return this->position; }

void CircleHitbox::setCenter(Vector2 position) { this->position = position; }
...
void ::CircleHitbox::renderHitbox() {
  ...
  int y;
  (this->active) ? SDL_SetRenderDrawColor(Main::renderer, 0, 0, 0, 255)
...
void SquareHitbox::renderHitbox() {
  Uint8 r, g, b, a;
  SDL_GetRenderDrawColor(Main::renderer, &r, &g, &b, &a);
  iVector2 dim = {Main::convertTileSizeToPixelSize(this->w),
                  Main::convertTileSizeToPixelSize(this->h)};
...
SquareHitbox::SquareHitbox(Vector2 position, float width, float height)
    : Hitbox::Hitbox(hitboxType::SQUARE) {
  this->position = position;
  this->w = width;
  this->h = height;
}
...
bool SquareHitbox::collidesWith(Vector2 pos) {
  if (this->active) {
    if (pos.X >= this->position.X && pos.Y >= this->position.Y) {
      if (pos.X <= this->position.X + this->w &&
          pos.Y <= this->position.Y + this->h)
        return true;
    }
  }
  return false;
...
bool SquareHitbox::collidesWith(Hitbox *inputHitbox) {
  if (this->active) {
    ...
    case (hitboxType::SQUARE): {
      ...
      Vector2 centerA = {this->position.X + (this->w / 2),
                         this->position.Y + (this->h / 2)};
      Vector2 centerB = {sbox->position.X + (sbox->w / 2),
                         sbox->position.Y + (sbox->h / 2)};
      if (abs(centerA.X - centerB.X) < halfsumXside &&
          abs(centerA.Y - centerB.Y) < halfsumYside)
        return true;
    } break;

    case (hitboxType::CIRCLE): {
      Vector2 comparePos;
      Vector2 squareCenter = {this->position.X + (this->w / 2),
                              this->position.Y + (this->h / 2)};
      CircleHitbox *cbox = dynamic_cast<CircleHitbox *>(inputHitbox);
      // check if inside square
      if (cbox->position.X > this->position.X &&
          cbox->position.X < this->position.X + this->w &&
          cbox->position.Y > this->position.Y &&
          cbox->position.Y < this->position.Y + this->h)
        return true;
      if (cbox->position.X <= squareCenter.X)
        comparePos.X = this->position.X;
      // check for outside square
      else if (cbox->position.X > squareCenter.X)
        comparePos.X = this->position.X + this->w;

      if (cbox->position.Y <= squareCenter.Y)
        comparePos.Y = this->position.Y;
      else if (cbox->position.Y > squareCenter.Y)
        comparePos.Y = this->position.Y + this->h;

      if (cbox->position.distance(comparePos) < cbox->r)
        return true;
    } break;
    }
  }
    ...
    for (int x = this->position.X; x < this->position.X + this->w; x++) {
      for (int y = this->position.Y; y < this->position.Y + this->h; y++) {
        if (Main::world->tileAt(x, y) != nullptr &&
            Main::world->tileAt(x, y)->isSolid)
          if (this->collidesWith(Main::world->tileAt(x, y)))
            tile.push_back(Main::world->tileAt(x, y));
      }
    }
  }
  return tile;
...
RotatableHitbox::RotatableHitbox(Vector2 center, double w, double h)
    : Hitbox(hitboxType::ROTATABLE) {
...
double RotatableHitbox::getRotation() { return rotation; }
...
bool RotatableHitbox::collidesWithRotatable(RotatableHitbox *sq) {
  for (int i = 0; i < corners.size(); i++) {
    ...
    for (int i = 0; i < corners.size(); i++) {
      ...
      Vector2 next =
          (i == corners.size() - 1) ? sq->corners[0] : sq->corners[i + 1];
      Line l2 = {first, next};
...
void RotatableHitbox::setCenter(Vector2 position) { this->position = position; }
...
bool RotatableHitbox::collidesWith(Line line) {
  for (int i = 0; i < corners.size(); i++) {
    ...
    Vector2 next = (i == corners.size() - 1) ? corners[0] : corners[i + 1];
    Line l2 = {first, next};
    if (l2.checkIfIntersects(line))
      return true;
  }
  return false;
}

Vector2 RotatableHitbox::getCenter() { return this->position; }
...
bool RotatableHitbox::collidesWithSq(SquareHitbox *sq) {
  ...
  for (Vector2 corner : sq->getCorners()) {
    ...
    if (newCorner.X < bottmRight.X && newCorner.X > topLeft.X &&
        newCorner.Y < bottmRight.Y && newCorner.Y > topLeft.Y)
      return true;
  }
  return false;
}
...
bool RotatableHitbox::collidesWith(Vector2 pos) {
  ...
  this->setRotation(rotation);
  if (pos.X > topLeft.X && pos.X < bottomRight.X && pos.Y < bottomRight.Y &&
      pos.Y > topLeft.Y)
    return true;
  return false;
}
...
bool RotatableHitbox::collidesWith(Hitbox *inputHitbox) {
  switch (inputHitbox->type) {
  case (hitboxType::ROTATABLE):
    return this->collidesWithRotatable(
        dynamic_cast<RotatableHitbox *>(inputHitbox));
    break;
  case (hitboxType::SQUARE):
  ...
  case (hitboxType::CIRCLE):
    ...
    double rotation = this->getRotation();
    Vector2 circlePos =
        Main::rotatePt(cbox->position, this->position, -rotation);
    this->setRotation(0);

    // check if circle is inside square
    Vector2 &topLeft = corners[0];
    Vector2 &bottomRight = corners[2];
    if (circlePos.X < bottomRight.X && circlePos.X > topLeft.X &&
        circlePos.Y < bottomRight.Y && circlePos.Y > topLeft.Y) {
      this->setRotation(rotation);
      return true;
    }

    // get side circle is on
    double x = (circlePos.X < this->position.X)
                   ? this->position.X - (this->width / 2)
                   : this->position.X + (this->width / 2);
    double y = (circlePos.Y < this->position.Y)
    ...
    for (int x = start.X; x <= end.X; x++) {
      for (int y = start.Y; y <= end.Y; y++) {
        if (Main::world->tileAt(x, y) != nullptr &&
            Main::world->tileAt(x, y)->isSolid)
          if (this->collidesWith(Main::world->tileAt(x, y)))
            tiles.push_back(Main::world->tileAt(x, y));
      }
    }
  }
  return tiles;
