...
bool Camera::setX(double X, bool renderScrOnMove) {
  ...
  if (bounds.bottomLeft.X < 0)
    X += abs(bounds.bottomLeft.X);
  else if (bounds.bottomRight.X >= Main::world->WORLD_WIDTH + 1)
    X -= bounds.bottomRight.X - (Main::world->WORLD_WIDTH + 1);
  this->X = X;
  updateCameraBounds();
  if (renderScrOnMove)
    renderScreen();
...
bool Camera::setY(double Y, bool renderScrOnMove) {
  ...
  if (bounds.topLeft.Y < 0)
    Y += abs(bounds.topLeft.Y);
  else if (bounds.bottomLeft.Y >= Main::world->WORLD_HEIGHT)
    Y -= bounds.bottomLeft.Y - Main::world->WORLD_HEIGHT;
  this->Y = Y;
  updateCameraBounds();
  if (renderScrOnMove)
    renderScreen();
...
Camera::Camera(SDL_Renderer *renderer, bool renderScr) {
  ...
  setPos(Main::world->WORLD_WIDTH / 2, Main::world->WORLD_HEIGHT / 2,
         renderScr);
}
...
bool Camera::renderTiles() {
  ...
  // Fetch world bounds safely
  const int maxWidth = Main::world->WORLD_WIDTH;
  const int maxHeight = Main::world->WORLD_HEIGHT;

  // Calculate grid bounds and clamp to valid world indices [0, max - 1]
  int startX = std::max(0, (int)std::floor(cameraBounds.bottomLeft.X));
  int endX = std::min(maxWidth - 1, (int)std::ceil(cameraBounds.bottomRight.X) +
                                        (int)zoomScale);

  int startY = std::max(0, (int)std::floor(cameraBounds.topLeft.Y));
  int endY = std::min(maxHeight - 1, (int)std::ceil(cameraBounds.bottomLeft.Y) +
                                         (int)zoomScale);

  for (int x = startX; x <= endX; x++) {
    for (int y = startY; y <= endY; y++) {

      // 1. Cache the pointer locally! Prevent redundant, unsafe lookups
      Tile *tile = Main::world->tileAt(x, y);

      // 2. Safe Null Check
      if (tile == nullptr)
        continue;

      // 3. Handle Multi-Tiles vs Regular Tiles
      if (tile->isMultiTile) {
        if (std::find(pos.begin(), pos.end(), Vector2(x, y)) == pos.end()) {
          if (tile->draw(renderer, *this)) {
            size++;
          }
          pos.push_back({x, y});
        }
...
bool Camera::renderTileWalls() {
  int size = 0;
  for (int x = std::floor(cameraBounds.bottomLeft.X);
       x <= std::ceil(cameraBounds.bottomRight.X) + zoomScale; x++) {
    for (int y = std::floor(cameraBounds.topLeft.Y);
         y <= std::ceil(cameraBounds.bottomLeft.Y) + zoomScale; y++) {
      if ((x >= 0 && y >= 0) &&
          (x <= Main::world->WORLD_WIDTH && y <= Main::world->WORLD_HEIGHT)) {
        if (Main::world->tileWallAt(x, y) != nullptr) {
          if (Main::world->tileWallAt(x, y)->draw(renderer, *this))
            size += 1;
        }
      }
    }
...
bool Camera::renderEntities() {
  for (Entity *entity : Main::entities)
    if (entity->renderToScreen && !entity->toBeDeleted() &&
        (Main::player == nullptr ||
         Main::player->center.distance(entity->center) <
             (this->diagonalLength / 2) +
                 (std::sqrt(std::pow(entity->width, 2) +
                            std::pow(entity->height, 2)) /
                  2))) {
      if (Main::player == nullptr || entity != Main::player->arm ||
          entity != Main::player->arm->getSwingItem()) {
        entity->renderEntity();
      }
    }
  return true;
...
Vector2 Camera::getPos() { return {(double)this->X, (double)this->Y}; }
...
void Camera::renderPlayer(Player *player) {
  player->renderEntity();
  if (player->arm->getSwingItem() != nullptr &&
      player->arm->getSwingItem()->active)
    player->arm->getSwingItem()->renderEntity();
...
void Camera::renderPlayerGUI(Player *player) {
  ...
  if (Main::player->inventoryOpen) {
    ...
    GUIinterface::renderPlayerHotbar(Main::player);
}

CornersRect Camera::checkCameraBoundsAt(Vector2 pos) {
  CornersRect bounds;
  double lowerY =
      pos.Y + (Main::convertPixelSizeToTileSize(Main::WINDOW_HEIGHT, this) / 2);
  double upperY =
      pos.Y - (Main::convertPixelSizeToTileSize(Main::WINDOW_HEIGHT, this) / 2);
  double upperX =
      pos.X + (Main::convertPixelSizeToTileSize(Main::WINDOW_WIDTH, this) / 2);
  double lowerX =
      pos.X - (Main::convertPixelSizeToTileSize(Main::WINDOW_WIDTH, this) / 2);
