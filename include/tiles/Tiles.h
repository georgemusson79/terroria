...
#pragma once
class Item;
enum tileIDEnum { DIRT, GRASS, STONE, WOOD, HCRYSTAL, CHEST, AIR };
...
public:
  ...
  void randomTickUpdate() override;
...
public:
  HeartCrystal(uint16_t X, uint16_t Y, bool placeIntoWorld = true,
               bool isParent = true, MultiTileObject *parent = nullptr);
  int x = 4;
  void update() override { std::cout << x; }
};
...
public:
  ...
  int columns = 9;
  bool isOpen = false;
  Chest(uint16_t X, uint16_t Y, bool placeIntoWorld = true,
        bool isParent = true, MultiTileObject *parent = nullptr);
