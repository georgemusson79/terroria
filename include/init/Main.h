...
enum skyBackgrounds { DEFAULT };

enum class cursorType { MANUAL, SMART };
...
extern bool
    doesBossBarExist; // only allows one boss bar to be rendered at a time
...
SDL_Rect getTextureFrame(int frameNumber, int totalFrames,
                         TextureInfo *textureInfo, int startX = 0,
                         int startY = 0);
SDL_Rect getTextureFrame(int frameNumber, int totalFrames, SDL_Texture *texture,
                         int textureWidth, int textureHeight, int startX = 0,
                         int startY = 0);
...
template <typename T> T *clone(T &obj) { return new T(obj); }
} // namespace Main
