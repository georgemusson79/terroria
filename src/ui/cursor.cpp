...
int Cursor::selectedTile() {
  ...
  return Main::world->tileAt(pos.X, pos.Y)->getID();
}

bool Cursor::swapItem(GUIinterface::InventoryTile *invTile) {
  if (invTile == nullptr && Cursor::item == nullptr)
    return false;
  else if (invTile->itemType != SpecialisedType::GENERIC &&
           Cursor::isHoldingItem && Cursor::item->itemType != invTile->itemType)
    return false;

  if (Cursor::isHoldingItem && invTile->item != nullptr &&
      Cursor::item->id == invTile->item->id &&
      invTile->item->count + Cursor::item->count <= invTile->item->maxStack) {
...
void Cursor::update() { Cursor::cursorWorldPos = Cursor::WorldPos(); }

bool Cursor::isLeftPressed() {
  int x;
  if (SDL_GetMouseState(&x, NULL) && SDL_BUTTON(SDL_BUTTON_LEFT))
    return true;
  return false;
}
bool Cursor::isRightPressed() {
  int x;
  if (SDL_GetMouseState(&x, NULL) && SDL_BUTTON(SDL_BUTTON_RIGHT))
    return true;
  return false;
}
...
bool Cursor::dropItem(Player *player) {
  if (Cursor::isHoldingItem) {
    ...
    Vector2 normalised =
        (Cursor::WorldPos() - player->position) / Vector2(length, length);
