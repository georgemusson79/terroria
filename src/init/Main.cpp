    ...
    while (Debug::running)
    {
        ...
        for (int e = 0; e < Main::entities.size(); e++)
        {
            if (Main::entities[e] != nullptr && !Main::entities[e]->toBeDeleted() && Main::entities[e]->active)
            {
                ...
                float maxUpdateDist = Main::camera->diagonalLength + std::sqrt(std::pow(entity->width, 2) + std::pow(entity->height, 2));
                if (Main::entities[e]->alwaysUpdate || (Main::player == nullptr || Main::entities[e]->center.distance(Main::player->center)) < maxUpdateDist)
