...
void Main::init() {
  ...
  Main::window = SDL_CreateWindow("Terroria", SDL_WINDOWPOS_CENTERED,
                                  SDL_WINDOWPOS_CENTERED, Main::WINDOW_WIDTH,
                                  Main::WINDOW_HEIGHT,
                                  SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
  if (Debug::vsync)
    Main::renderer = SDL_CreateRenderer(
        Main::window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
  else {
    std::cout << "vsync false\n";
    Main::renderer =
        SDL_CreateRenderer(Main::window, -1, SDL_RENDERER_ACCELERATED);
  }
...
void Main::initAllTextures() {
  ...
  Main::tileTexture.insert(
      {STONE, IMG_LoadTexture(Main::renderer, "assets\\tiles\\stone.png")});
  Main::tileTexture.insert(
      {WOOD, IMG_LoadTexture(Main::renderer, "assets\\tiles\\wood.png")});
  Main::tileTexture.insert(
      {HCRYSTAL, IMG_LoadTexture(Main::renderer, "assets\\tiles\\heart.png")});
  Main::backgrounds.insert(
      {DEFAULT, IMG_LoadTexture(Main::renderer, "assets\\tiles\\sky.png")});
  GUIinterface::inventoryTileTexture = std::shared_ptr<TextureInfo>(
      Main::loadTextureWithInfo("assets\\GUI\\InventoryBackgrounds.png"));
  GUIinterface::heartTexture =
      IMG_LoadTexture(Main::renderer, "assets\\GUI\\heart.png");
  GUIinterface::manaTexture =
      IMG_LoadTexture(Main::renderer, "assets\\GUI\\mana.png");
