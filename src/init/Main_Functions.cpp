...
TextureInfo *Main::loadTextureWithInfo(std::string texture)
{
    ...
    if (info->texture != NULL)
        SDL_QueryTexture(info->texture, NULL, NULL, &info->textureWidth, &info->textureHeight);
    return info;
}
...
{
    if (pos >= Vector2(0, 0) && pos.X <= Main::WORLD_WIDTH && pos.Y <= Main::WORLD_HEIGHT)
        return true;
    return false;
}
...
{
    if (Main::inWorldBounds(pos) and Main::tiles[pos.X][pos.Y] != nullptr and Main::tiles[pos.X][pos.Y]->getID() != AIR)
        return true;
    return false;
}
