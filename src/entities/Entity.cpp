...
void Entity::update() {
  ...
  if (this->damage > 0) {
    ...
    if (Main::player != nullptr && this->collidesWith(Main::player))
      this->onHitPlayer(Main::player, this);
  }
}
...
bool Entity::onHitPlayer(Player *player, Entity *src) {
  if (this->hostile)
    return player->hurt(this->damage, this->kbDealt, src);
  return false;
}

bool Entity::setX(double X) {
  if (X + this->width <= Main::world->WORLD_WIDTH + 1 && X > 0) {
...
bool Entity::setY(double Y) {
  if (Y + this->height <= Main::world->WORLD_HEIGHT + 1 && Y > 0) {
    this->oldPos.Y = this->position.Y;
    this->oldPos.X = this->position.X;
    this->position.Y = Y;
...
bool Entity::setPos(double X, double Y) {
  if (X + this->width <= Main::world->WORLD_WIDTH + 1 && X > 0 &&
      Y + this->height <= Main::world->WORLD_HEIGHT + 1 && Y > 0) {
    this->oldPos.X = this->position.X;
    this->oldPos.Y = this->position.Y;
    this->position.Y = Y;
    this->position.X = X;
...
bool Entity::renderEntity() {
  ...
  Vector2 dims = {Main::convertTileSizeToPixelSize(this->width * this->scale),
                  Main::convertTileSizeToPixelSize(this->height * this->scale)};
  SDL_Rect r = {renderPos.X, renderPos.Y, dims.X, dims.Y};
  SDL_Rect partToRender = Main::getTextureFrame(
      this->animationFrame, this->frameCount, this->texture, this->textureWidth,
      this->textureHeight, 0, 0);
  SDL_RenderCopyEx(Main::renderer, this->texture, &partToRender, &r,
                   this->rotation, NULL, this->getSpriteDirection());
  if (Debug::renderHitboxes)
    this->renderHitboxes();
  return true;
}

bool Entity::toBeDeleted() { return this->markForDeletion; }

float Entity::getDistanceTravelled() {
  return std::sqrt(std::pow(distanceTravelled.X, 2) +
                   std::pow(distanceTravelled.Y, 2));
}
...
SDL_RendererFlip Entity::getSpriteDirection() {
  ...
  if (this->hDirection != 1 && this->canFlipH)
    flipType = SDL_FLIP_HORIZONTAL;
  if (this->vDirection != 1 && this->canFlipV)
    flipType = SDL_FLIP_VERTICAL;
  if (this->hDirection != 1 && this->canFlipH && this->canFlipV &&
      this->vDirection != 1)
    flipType = (SDL_RendererFlip)(SDL_FLIP_HORIZONTAL | SDL_FLIP_VERTICAL);
  return flipType;
}

Entity::Entity(Vector2 position, float width, float height, int health,
               std::string pathToTexture, bool gravity, bool defaultCollider,
               bool tileCollision)
    : position(position), width(width), height(height), gravity(gravity),
      tileCollision(tileCollision), health(health), maxHealth(health) {
  this->setTexture(pathToTexture);
  setPos(position.X, position.Y);
  if (defaultCollider)
    hitboxes.push_back(
        new SquareHitbox(this->position, this->width, this->height));
  this->rotationPoint = {Main::convertTileSizeToPixelSize(width) / 2,
                         Main::convertTileSizeToPixelSize(height) / 2};
  Main::entitiesToSpawn.push_back(this);
}

void Entity::addToEntitiesList() { Main::entities.push_back(this); }

Entity::Entity(Vector2 position, float width, float height, int health,
               SDL_Texture *t, bool gravity, bool defaultCollider,
               bool tileCollision)
    : position(position), width(width), height(height), gravity(gravity),
      tileCollision(tileCollision), health(health), maxHealth(health) {
  this->texture = t;
...
bool Entity::switchFrames(int frame) {
  if (frame<this->frameCount &&this->timeBetweenFrames> this
...
bool Entity::collidesWith(Vector2 position) {
  for (Hitbox *hitbox : this->hitboxes)
    if (hitbox->collidesWith(position))
      return true;
  return false;
}
...
bool Entity::collidesWith(Hitbox *inputHitbox) {
  if (this->entityCollision)
    for (Hitbox *entityHitbox : this->hitboxes)
      if (entityHitbox->collidesWith(inputHitbox))
        return true;
  return false;
}
...
bool Entity::collidesWith(Tile *tile) {
  if (tile->isSolid) {
    for (Hitbox *entityHitbox : this->hitboxes)
      if (entityHitbox->collidesWith(tile))
        return true;
  }
  return false;
}
...
bool Entity::collidesWith(Entity *entity) {
  if (entity == nullptr || entity->getKilled() || this->getKilled() ||
      this->toBeDeleted() || entity->toBeDeleted() ||
      !entity->entityCollision || this->entityCollision == false ||
      !this->active || !entity->active)
    return false;
  for (Hitbox *hitbox : entity->hitboxes)
...
void Entity::setTexture(std::string path) {
  ...
  SDL_QueryTexture(this->texture, NULL, NULL, &this->textureWidth,
                   &this->textureHeight);
}
...
bool Entity::getKilled() { return this->killed; }

bool Entity::hurt(int dmg, float kb, Entity *src) {
  if (this->maxHealth == -1 or this->dmgImmuneMaxTime == -1)
    return false;
  this->checkDmgImmune();
  ...
  float kbDirection = (src == nullptr)                       ? 0
                      : (this->center.X - src->center.X > 0) ? 1
                                                             : -1;
...
void Entity::checkDmgImmune() {
  if (this->invulnerable) {
    if (SDL_GetTicks() - dmgImmuneTime > dmgImmuneMaxTime &&
        dmgImmuneMaxTime != -1)
...
bool Entity::checkOnSameTeam(Entity *e) {
  if (this->hostile && e->hostile)
    return true;
  if (e->friendly && this->friendly)
    return false;
  return false;
}

void Entity::walk(Entity *e, Vector2 to, double speed, int firstWalkFrame,
                  int lastWalkFrame, int standingFrame) {
  if (e->position.distance(to) > 1) {
    if (e->onGround) {
      ...
      e->switchFrames((e->animationFrame == lastWalkFrame)
                          ? firstWalkFrame
                          : e->animationFrame + 1);
    }
  } else {
