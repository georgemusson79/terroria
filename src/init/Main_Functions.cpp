...
TextureInfo *Main::loadTextureWithInfo(std::string texture) {
  ...
  if (info->texture != NULL)
    SDL_QueryTexture(info->texture, NULL, NULL, &info->textureWidth,
                     &info->textureHeight);
  return info;
}

SDL_Rect Main::getTextureFrame(int frameNumber, int totalFrames,
                               TextureInfo *textureInfo, int startX,
                               int startY) {
  return Main::getTextureFrame(frameNumber, totalFrames, textureInfo->texture,
                               textureInfo->textureWidth,
                               textureInfo->textureHeight);
}
SDL_Rect Main::getTextureFrame(int frameNumber, int totalFrames,
                               SDL_Texture *texture, int textureWidth,
                               int textureHeight, int startX, int startY) {
  ...
  if (e->type == SDL_KEYDOWN) {
    if (Main::player != nullptr) {
      if (Debug::runDebugFunctions)
        Debug::DebugHotkeyFunctions(e->key.keysym.sym);
      ...
      if (e->key.keysym.sym == SDLK_b) {
        ...
        ItemPickup *i4 =
            new ItemPickup(std::shared_ptr<Item>(new MusketBallItem),
                           Main::player->position - Vector2(2, 2));
        i4->item->setCount(99);
        i4 = new ItemPickup(std::shared_ptr<Item>(new ArrowItem),
                            Main::player->position - Vector2(2, 2));
        i4->item->setCount(99);
        new ItemPickup(std::shared_ptr<Item>(new TestSword),
                       Main::player->position - Vector2(2, 2));
        ItemPickup *i = new ItemPickup(std::shared_ptr<Item>(new Pistol),
                                       Main::player->position - Vector2(2, 2));
        ItemPickup *i2 = new ItemPickup(std::shared_ptr<Item>(new WoodBow),
                                        Main::player->position - Vector2(2, 2));
        ItemPickup *u3 =
            new ItemPickup(std::shared_ptr<Item>(new CopperPickaxe),
                           Main::player->position - Vector2(2, 2));
        // //i2->item->count = 999;
        // //new Zombie(Cursor::WorldPos());
      ...
      if (e->key.keysym.sym == SDLK_DELETE) {
        for (auto e : Main::entities)
          if (e != Main::player->arm)
            e->despawn();
      }

      else if (e->key.keysym.sym == SDLK_F11) {
        if (Debug::fullscreen)
          SDL_SetWindowFullscreen(Main::window, SDL_WINDOW_FULLSCREEN_DESKTOP);
        else
          SDL_SetWindowFullscreen(Main::window, 0);
...
void Main::setCursorType(cursorType type) {
  ...
  SDL_Surface *s;
  s = (type == cursorType::SMART)
          ? IMG_Load("assets\\cursor\\smart_cursor.png")
          : IMG_Load("assets\\cursor\\manual_cursor.png");
...
void Main::removeDeletedTiles() {
  std::vector<Tile *> list(Main::tilesToDelete.begin(),
                           Main::tilesToDelete.end());
  while (!list.empty()) {
    if (list.size() > 0) {
      delete list.back();
      list.pop_back();
...
double Main::toDegrees(double val) { return val * (180 / M_PI); }
double Main::toRadians(double val) { return val * (M_PI / 180); }
...
void Main::updateLightMap(Vector2 position, int lightStrength) {
  ...
  for (int x = topLeft.X; x < position.X + maxDistance; x++) {
    for (int y = topLeft.Y; y < position.Y + maxDistance; y++) {
      ...
      if (x > 0 && y > 0 && x <= Main::world->WORLD_WIDTH &&
          y <= Main::world->WORLD_HEIGHT && distance < maxDistance) {
...
void Main::resetLightMap() {
  for (auto point : Main::changedLights)
    Main::lightMap[point.X][point.Y] = Main::defaultLightValue;
  Main::changedLights = {};
}

bool Main::inWorldBounds(Vector2 pos) {
  if (pos >= Vector2(0, 0) && pos.X <= Main::world->WORLD_WIDTH &&
      pos.Y <= Main::world->WORLD_HEIGHT)
    return true;
  return false;
}
...
void Main::renderPos(Vector2 pos) {
  ...
  SDL_RenderDrawLine(Main::renderer, pos.X - 10, pos.Y - 10, pos.X + 10,
                     pos.Y + 10);
...
void Main::drawSquare(Vector2 center, SDL_Color color, int width) {
  ...
  SDL_Rect rect = {center.X - (width / 2), center.Y - (width / 2), width,
                   width};
...
Vector2 Main::rotatePt(Vector2 in, Vector2 around, double rotation) {
  ...
  matrix[1][0] = sin(rad);
  matrix[1][1] = cos(rad);
  Vector2 res = {matrix[0][0] * org.X + matrix[0][1] * org.Y,
                 matrix[1][0] * org.X + matrix[1][1] * org.Y};
  res += around;
  return res;
}

bool Main::checkForTile(Vector2 pos) {
  if (Main::inWorldBounds(pos) and
      Main::world->tileAt(pos.X, pos.Y) != nullptr and
      Main::world->tileAt(pos.X, pos.Y)->getID() != AIR)
    return true;
  return false;
}
...
void Main::spawnEntities() {
  ...
  std::sort(Main::entities.begin(), Main::entities.end(),
            [](Entity *a, Entity *b) {
...
Tile *Main::getTileAt(int x, int y) {
  if (Main::checkForTile({x, y}))
    return Main::world->tileAt(x, y);
  return nullptr;
}
  ...
  for (int x = 0; x <= Main::world->WORLD_WIDTH; x++) {
    for (int y = 0; y <= Main::world->WORLD_HEIGHT; y++) {
      if (Main::world->tileAt(x, y) != nullptr) {
        Main::world->tileAt(x, y)->save(file);
        file.close();
        return;
      }
    }
