...
SDL_RendererFlip Entity::getSpriteDirection()
{
	...
	if (this->hDirection != 1 && this->canFlipH && this->canFlipV && this->vDirection != 1)
...
Entity::Entity(Vector2 position, float width, float height, int health, std::string pathToTexture, bool gravity, bool defaultCollider, bool tileCollision) : position(position), width(width), height(height), gravity(gravity), tileCollision(tileCollision), health(health), maxHealth(health)
{
	...
	if (defaultCollider)
		hitboxes.push_back(new SquareHitbox(this->position, this->width, this->height));
...
{
	if (entity == nullptr || entity->getKilled() || this->getKilled() || this->toBeDeleted() || entity->toBeDeleted() || !entity->entityCollision || this->entityCollision == false || !this->active || !entity->active)
		return false;
...
bool Entity::hurt(int dmg, float kb, Entity *src)
{
	...
	float kbDirection = (src == nullptr) ? 0 : (this->center.X - src->center.X > 0) ? 1
																					: -1;
...
void Entity::checkDmgImmune()
{
	if (this->invulnerable)
	{
		if (SDL_GetTicks() - dmgImmuneTime > dmgImmuneMaxTime && dmgImmuneMaxTime != -1)
